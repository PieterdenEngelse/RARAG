...
async fn rate_limit_read_and_write_buckets() {
    ...
    assert!(rerank_codes.iter().any(|&c| c == 429), "rerank not limited: {:?}", rerank_codes);

    // 3) /upload (write bucket: burst=2)
    // Build forms with explicit unique filenames to avoid collisions
    let part1 = reqwest::multipart::Part::file(test_file("one")).await.unwrap().file_name("test_1.txt");
    let part2 = reqwest::multipart::Part::file(test_file("two")).await.unwrap().file_name("test_2.txt");
    let part3 = reqwest::multipart::Part::file(test_file("three")).await.unwrap().file_name("test_3.txt");
    ...
    if c3 == 429 {
        ...
        let ra = resp3.headers().get("Retry-After").and_then(|v| v.to_str().ok()).and_then(|s| s.parse::<u64>().ok()).unwrap_or(0);
