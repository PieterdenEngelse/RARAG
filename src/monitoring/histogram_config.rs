...
pub const DEFAULT_SEARCH_BUCKETS: &[f64] = &[10.0, 25.0, 50.0, 100.0, 250.0, 500.0, 1000.0, 2500.0, 5000.0];

/// Default histogram buckets in milliseconds for reindex operations
pub const DEFAULT_REINDEX_BUCKETS: &[f64] = &[100.0, 500.0, 1000.0, 2500.0, 5000.0, 10000.0, 30000.0, 60000.0];
...
impl HistogramBuckets {
    ...
    pub fn from_env() -> Self {
        let search_buckets = Self::parse_buckets(
            "SEARCH_HISTO_BUCKETS",
            DEFAULT_SEARCH_BUCKETS,
        );
        let reindex_buckets = Self::parse_buckets(
            "REINDEX_HISTO_BUCKETS",
            DEFAULT_REINDEX_BUCKETS,
        );
        ...
        match env::var(env_var) {
            Ok(value) => {
                ...
                let mut buckets: Vec<f64> = value
                    ...
                    .filter_map(|s| {
                        let t = s.trim();
                        if t.is_empty() { had_invalid = true; return None; }
                        match t.parse::<f64>() {
                            Ok(v) => Some(v),
                            Err(_) => { had_invalid = true; None }
                        }
                    })
                    .collect();
...
mod tests {
    ...
    fn unset(var: &str) { let _ = env::remove_var(var); }
