...
impl ConfigurableMetricsRegistry {
    ...
    pub fn create_search_histogram(&self, name: &str, help: &str) -> prometheus::Result<Histogram> {
        let opts =
            HistogramOpts::new(name, help).buckets(self.histogram_config.search_buckets.clone());
    ...
    /// - help: Help text describing the metric
    pub fn create_reindex_histogram(
        &self,
        name: &str,
        help: &str,
    ) -> prometheus::Result<Histogram> {
        let opts =
            HistogramOpts::new(name, help).buckets(self.histogram_config.reindex_buckets.clone());
        let histogram = Histogram::with_opts(opts)?;
        self.registry.register(Box::new(histogram.clone()))?;
    ...
    pub fn histogram_config(&self) -> &HistogramBuckets {
        &self.histogram_config
    }
}

impl Default for ConfigurableMetricsRegistry {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_search_histogram_uses_configured_buckets() {
        ...
        let h = reg
            .create_search_histogram("search_latency_ms_test", "test")
            .expect("histogram");
        h.observe(1.5);
        h.observe(3.0);
        ...
        assert!(metrics
            .iter()
            .any(|m| m.get_name() == "search_latency_ms_test"));
    ...
    fn test_duplicate_metric_registration_fails() {
        let reg = ConfigurableMetricsRegistry::new();
        let _ = reg
            .create_search_histogram("dup_metric", "first")
            .expect("first ok");
        let dup = reg.create_search_histogram("dup_metric", "second");
        assert!(
            dup.is_err(),
            "expected registration error for duplicate metric name"
        );
    }
}
